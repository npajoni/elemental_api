...
p = ElementalProfile('13')
p.OutputGroupName = 'apple_live_group_settings'
x = Elemental('10.4.0.91')
#for job in GetJobList(x):
#    print job.id, job.status, job.progress, job.priority, job.average_fps

#try:
#    job = ElementalJob(x,'34')
#except ElementalError as e:
#    print e
    
#job.getJobStatus(True)
#print job.error_messages
#job.loadJob()
#j = ElementalJob(x, p, '/mnt/input/VID_20160108_120046358.mp4', '/mnt/output/0001/')
#j.id = '35'
#job_status = j.getJobStatus(x)

#print job_status
#print x.GetJobStatus('35')
try:
    job = CreateJobFromProfile(x, '/mnt/input/VID_20160108_120046358.mp4', '/mnt/output/kaka/', 'test_kaka' , p)
    while (job.status == 'pending' or job.status == 'preprocessing' or job.status == 'running'):
	print job.getJobStatus(True), job.getJobProgress(), job.getJobPriority(), job.getJobElapsed(), job.getJobAvgFps(), job.error_messages, job.server.server	
	time.sleep(5)
except ElementalError as e:
    print e
print job.file_input
print job.getOutputFiles(True)
...
#except ElementalError as e:
#    print e
#print job.getJobStatus(True)
#try:

#job = ElementalJob(x,'35')
#job.getJobStatus(True)
#try:
#    job.cancelJob()
#except ElementalError as e:
